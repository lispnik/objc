...
(defun %shim-source (result-node arg-nodes index function-name)
  "The Lisp source for a hand-written C shim callable, as a string."
  (let* ((*trampoline-counter* 0)
         (definitions '())
         (structp (struct-node-p result-node))
         (result-c (multiple-value-bind (type more)
    ...
    (with-output-to-string (out)
      (format out ";;;; Generated by objc for one aggregate-carrying signature.~%")
      (format out "(in-package #:objc)~%~%")
      (format out "(ffi:clines~%  \"#include <string.h>\"")
      (dolist (definition (reverse definitions))
        (format out "~%  ~s" definition))
      ;; The shim itself. No `at' sign may appear anywhere ECL will read as its
      ;; own return syntax, which is why the result is assigned rather than
      ;; produced by a return macro.
      (format out "~%  ~s"
      ...
      (format out "(defun ~a-address ()~%" function-name)
      (format out "  (ffi:c-inline () () :pointer-void~%")
      ;; The at-sign return syntax is ECL's own and belongs here; it is other
      ;; uses of it in a c-inline body that ECL misreads.
      (format out "    \"{ ~a(return) = (void *)~a; }\"~%" #\@ shim)
      (format out "    :one-liner nil))~%"))))
...
BODY is called as
  ...
  (unless (compiled-trampolines-available-p)
    (%unsupported (format nil "BUILD-~:@(~a~)" noun)
                  "an IMP is a real C function and there is no C compiler here"))
